...
class VideoIcon {
	constructor(layout, game, type) {
		this._imgContainer = $("<div>").addClass("face").addClass(type + "Face");
	...
	update(o, liveId) {
		...
		this._imgContainer.removeClass("active").removeClass("muted").removeClass("dead");
				...
				if (this._face._dead) {
					...
					acquire.acquire(function(id, localStream) {
						...
						if (localStream._dead) {
							that._game.logManager.log("No " + that.type + " device available");
							that._imgContainer.addClass("dead");
							
							if (!that._face._dead) {
								that._face = localStream;
								that._game.friendFaces.videoUpdateThisFace(that.type, localStream);
							}
						} else {
		...
		acquire._current = {
			...
			get: function(callback) {
				...
				if (!that.getting) {
					...
					if (navigator.mediaDevices === undefined) { // Not on https
...
//TODO iOS when acquire video, acquire again audio if previously acquired (f*ck iOS)
					navigator.mediaDevices.getUserMedia({ video: (type === "video") ? {
						width: 320,
						height: 240,
						facingMode: "user"
					} : false, audio: true })
						.then(function(localStream) {
							...
							if (type === "video") {
								...
								localStream._size = { width: settings.width, height: settings.height };
							}
