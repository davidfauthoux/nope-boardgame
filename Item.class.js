...
class Item {
	constructor(game, kind) {
		...
		this._createInstance = function(layout, countable, liveId) {
			...
			itemDiv = layout;
			itemDiv.$.addClass("item").addClass("item-" + kind).attr("data-kind", kind);
			...
			var innerLayout = countable ? itemDiv.packed().horizontal().add() : itemDiv;
			...
			if (kind === "live") {
				...
				if (countable) {
					foundContentDiv.find("[data-location]").append(function() {
						...
						var extraSpot = new Spot(extraSpotLayout, game, extraSpotLocation, { extra: true });
						...
						if (prevSpot !== null) {
							prevSpot.eachItemInstances(function(ii) {
								...
								extraSpot.addItem(ii.item.kind, ii.infinite ? undefined : ii.count, ii.liveId);
				...
				foundContentDiv.find("input").first().replaceWith(function() {
					defaultInputVal = $(this).val();
					var titleDiv = $("<div>").addClass("editable").text(defaultInputVal);
					...
					onClick = function() {
						...
						input.on("keydown", function(e) {
							...
							if (e.key === "Enter") {
								...
								}, null, countItem.spot, countItem.item);
			...
			countItem = {
				...
				_updateCountDiv: function() {
					...
					if (((countItem.count === 1) || countItem.infinite) && (that.properties.count === undefined) && (countItem.state.count === undefined)) {
