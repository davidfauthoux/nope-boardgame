...
class FriendMouse {
	constructor(game) { //, number) {
		...
		var mouse = $("<svg style=\"filter: drop-shadow(2px 2px 3px rgba(0, 0, 0, 0.5)); fill: white; stroke: black; stroke-width: 28; stroke-linecap: round; stroke-linejoin: round; stroke-miterlimit: 10;\" width=\"20px\" height=\"20px\" viewBox=\"0 0 385 462\"><polygon points=\"17,18 359,212 245,257 361,372 279,437 189,297 120,393\"/></svg>");
		...
		if (this._currentRef !== null) {
			...
			if (p) {
				...
				if (!this._visible) {
					...
					this._mouseDiv.stop().animate({ top: yy, left: xx }, (0.15 / 2) * 1000); // Based on the default throttle (should not be too slow or the cursor will jump)
				}
			}
		}
	}
