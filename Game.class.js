...
class Game {
	constructor() {
		...
		if (!DomUtils.browser(required)) {
			...
			warningOverlay.horizontal().add().$.addClass("warningText").html("<div>Browser not compatible</div>" + /*"<div>" + navigator.userAgent + "</div>" + */"<div>Please update to " + required + "</div>");
			UserInteraction.get().click(warningOverlay.horizontal().add().$.addClass("warningClose"), function() {
		...
		loadingOverlay.layout().north().layout().west().$.text("Loading...").addClass("loading");
		...
		var _download = function(server, prefix, files, i, defaults, contents, callback) {
			...
			if (file.endsWith(".xml") || file.endsWith(".css") || file.endsWith(".js") || file.endsWith(".json")) {
				...
				server.download(new Heap(((prefix === null) ? "" : prefix) + file), contentHeap).err(function(e) {
		...
		var listAndDownload = function(server, resourceDir, contents, callback) {
			...
			server.list(new Heap(resourceDir), listHeap).err(function(e) {
				...
				_download(server, resourceDir + "/", list, 0, null, {}, function(listContents) {
