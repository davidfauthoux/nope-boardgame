	...
	constructor(layout, game) {
		...
		var parseEvent = function(event, received, doIt) {
			...
			if (event.action === "pool") {
				if (doIt) {
					game.gameManager.newPool(event.layout, event.name, event.properties, event.floating);
				}
				return true;
			}
			...
			if (event.action === "paint") {
				...
				if (spot !== null) {
					if (doIt) {
						...
						return game.triggerManager.watchdog("paint", spot, itemInstance, null, { key: event.key, value: event.value });
					}
					return true;
				} else {
					return false;
			...
			if (event.action === "drop") {
				...
				if (spot !== null) {
					...
					if (doIt) {
						...
						var itemInstance = spot.getItemInstance(event.kind);
						return game.triggerManager.watchdog("drop", spot, itemInstance, null, { item: game.itemManager.getItem(event.kind), count: event.count });
					}
					//%% }
					//%% }
					return true;
			...
			if (event.action === "destroy") {
				...
				if (spot !== null) {
					...
					if (doIt) {
						...
						var itemInstance = spot.getItemInstance(event.kind);
						return game.triggerManager.watchdog("destroy", spot, itemInstance, null, { count: event.count });
					}
					//%% }
					//%% }
					return true;
				...
				if (event.location === event.to) {
					...
					if (spot !== null) {
						if (doIt) {
							...
							var itemInstance = spot.getItemInstance(event.kind);
							return game.triggerManager.watchdog("move", spot, itemInstance, spot, {});
						}
						return true;
					} else {
						return false;
					...
					if (itemInstance !== null) {
						...
						if (to !== null) {
							if (doIt) {
								...
								return game.triggerManager.watchdog("move", spot, itemInstance, to, { count: event.count });
							}
							return true;
						} else {
							return false;
			...
			if (event.action === "news") {
				if (doIt) {
					game.newsManager.appendLine(event.id, event.live, event.texts, event.reverse, event.reverseback);
				}
				return true;
			}
		...
		*/
		...
		var stack = function(event) {
			if (!stackActivated) {
				console.log("Not possible to save any event during the configuration script");
				return;
			}
			...
			server.stack(new Heap(event)).res(function() {
				hideSaving();
			}).err(function(err) {
				hideSaving(err);
			}).run();
