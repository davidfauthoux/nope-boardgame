...
class TriggerManager {
	constructor(game) {
		...
		this._runScriptsInSpot = function(from, spot, item, runScripts) {
			...
			if (spot !== null) {
				...
				if (item !== null) {
					...
					console.log("Checking if should run script for: " + item.kind + " in: " + spot.location);
					var checkAndExec = function(k) {
						that._dropTriggers.get(k).each(function(f) {
							console.log("Drop script triggered (" + item.kind + " in " + spot.location + ")");
							exec.push(f(from, foundItemInstance, spot, null));
						});
					};
					checkAndExec(item.kind);
					Utils.each(GeneralReference.getGeneralKinds(item.kind), function(generalKind) {
				...
				spot.eachItemInstances(function(i) {
					that._instantTriggers.get(i.item.kind).each(function(f) { //TODO Remove instant triggers if useless
						console.log("Instant script triggered (" + i.item.kind + " in " + spot.location + ")");
						exec.push(f(from, i, spot, null));
					});

					if (foundItemInstance !== null) {
						that._dropinTriggers.get(i.item.kind).each(function(f) {
							console.log("Dropin script triggered (" + i.item.kind + " in " + spot.location + ")");
							exec.push(f(from, foundItemInstance, spot, null));
						});
					}
				});
		...
		*/
		...
		this._runWatchdogScriptsInSpot = function(action, spot, itemInstance, to, context) {
			...
			var checkAndExec = function(k) {
				that._watchdogTriggers.get(k).each(function(f) {
					...
					console.log("Watchdog script triggered (" + itemInstance.item.kind + " as " + k + " in " + spot.location + ")");
					try {
						ExecutionContext.runScripts(game, [ f(spot, itemInstance, to, context) ]);
					} catch (exception) {
			...
			if (ok) {
				Utils.each(GeneralReference.getGeneralKinds(itemInstance.item.kind), function(generalKind) {
					if (!ok) {
						return;
					}
					checkAndExec(generalKind.kind);
	...
	*/
	...
	watchdog(action, spot, itemInstance, to, context) {
		return this._runWatchdogScriptsInSpot(action, spot, itemInstance, to, context);
	}
}
