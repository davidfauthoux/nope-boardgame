...
class ScriptExecution {
	constructor(layout, game, contents) {
		...
		var makeAnonymousTextItem = function(kind, text, properties) {
			...
			game.generalReference.setContent(textKind, $("<span>").addClass("generatedText").text(text), properties, {});
		...
		var makeAnonymousColorItem = function(kind, text, properties) {
			...
			game.generalReference.setContent(colorKind, $("<div>").addClass("generatedColor").css({ background: text }), properties, {});
		...
		var parseLayout = function() {
			var nextNameId = 0;
			game.gameManager.layouts = LayoutBuilder.build(contents.layout, layout, function(vars, applyToLayout, item) {
				...
				var parseDrop = function(c, spotToDropIn, states, usingKind, usingCount) {
					var kind = (usingKind === undefined) ? unvar(c.text().trim()) : usingKind;
					var count = (usingCount === undefined) ? ((c.attr("count") === undefined) ? undefined : parseInt(c.attr("count"))) : usingCount;
					dropsFoundInLayout.push(function(pools, tracks, grids) {
						var it = spotToDropIn(pools, tracks, grids).drop(kind, (count === null) ? undefined : count);
				...
				var parseElements = function(buildSpot, states, cc, anonymousItemCount, anonymousItemProperties) {
					cc.children().each(function(_, c) {
						...
						if (ctag === "text") {
							...
							var kind = makeAnonymousTextItem(null, text, anonymousItemProperties);
						...
						if (ctag === "color") {
							var text = c.text().trim();
							// Only one possible in the spot
							var kind = makeAnonymousColorItem(null, text, anonymousItemProperties);
							parseDrop(c, buildSpot(c), states, kind, anonymousItemCount);
							return;
						}
						/*%%
				...
				if (tag === "track") {
					var name = getName();
					var title = (item.children("title").length === 0) ? null : item.children("title").text().trim();
					var size = parseInt(item.children("size").text().trim());
					var vertical = item.attr("direction") === "vertical";
					var start = (item.children("start").length === 0) ? 0 : parseInt(item.children("start").text().trim());
					var step = (item.children("step").length === 0) ? 1 : parseInt(item.children("step").text().trim());
					var limit = (item.children("limit").length === 0) ? null : parseInt(item.children("limit").text().trim());
					var reversed = (item.children("reversed").length > 0);
					game.gameManager.createTrack(applyToLayout, name, title, size, vertical, start, step, limit, reversed);

					parseElements(function(c) {
						var level = parseInt(c.attr("level"));
						return function(pools, tracks, grids) { return tracks.get(name).level(level); };
					}, {}, item, 1, { overlay: true, invisible: true }); 
				...
				if (tag === "pool") {
					...
					parseElements(function(c) {
						return function(pools, tracks, grids) { return pools.get(name); };
					}, {}, item, null, { steady: true } ); 
				...
				if (tag === "grid") {
					...
					var name = getName();
					var width = (item.children("width").length === 0) ? 0 : parseInt(item.children("width").text().trim());
					var height = (item.children("height").length === 0) ? 0 : parseInt(item.children("height").text().trim());
					var auto = (item.children("auto").text().trim() === "true");
					game.gameManager.createGrid(applyToLayout, name, type, auto, width, height);

					parseElements(function(c) {
						var i = parseInt(c.attr("i"));
						var j = parseInt(c.attr("j"));
						return function(pools, tracks, grids) { return grids.get(name).cell(i, j); };
					}, {}, item, null, { overlay: true, invisible: true } ); 
		...
		var buildScript = function(script) {
			...
			var methods = [ "global", "loop", "each", "random", "pools", "tracks", "grids", "spot", "from", "here", "hover", "log", "nop", "play", "mark", "layout", "pool" ];
		...
		var addStateCss = function(cssComposition, kind, states) {
			Utils.each(states, function(stateValues, stateKey) {
				Utils.each([ " > div > div > ", " > " ], function(chain) {
					cssComposition.css(".item-" + kind + chain + ".content.if-" + stateKey + " { display: none; }");
					cssComposition.css(".item-" + kind + chain + ".content.if-" + stateKey + "-_ { display: block ; }");
					Utils.each(stateValues, function(v) {
						if (v === null) {
							return;
						}
						cssComposition.css(".item-" + kind + ".state-" + stateKey + "-" + v + chain + ".content.if-" + stateKey + "-" + v + " { display: block; }");
						cssComposition.css(".item-" + kind + ".state-" + stateKey + "-" + v + chain + ".content.if-" + stateKey + "-_ { display: none; }");
					});
				});
			});
		};
		...
		var parseDeclarations = function() {
			...
			$($.parseXML(contents.declare)).children().first().children().each(function(_, item) {
				item = $(item);
				var tag = item.prop("tagName");
				if (!tag) {
					return null;
				...
				if (tag === "palette") {
					item.children().each(function(_, i) {
						...
						cssComposition.css(".item._-" + colorId + " > svg { fill: " + colorValue + " !important; }");
						cssComposition.css(".item._-" + colorId + " > span { background: " + colorValue + " !important; }");
						cssComposition.css(".item._-" + colorId + " > div > div > svg { fill: " + colorValue + " !important; }");
						cssComposition.css(".item._-" + colorId + " > div > div > span { background: " + colorValue + " !important; }");
						cssComposition.css(".spot.state-_-" + colorId + " > .underlay > svg { stroke: " + colorValue + " !important; }");
						cssComposition.css(".spot.state-_-" + colorId + " > .underlay > svg { fill: " + colorValue + " !important; }");
						cssComposition.css(".spot.state-border-" + colorId + " > .underlay > svg { stroke: " + colorValue + " !important; }");
						cssComposition.css(".spot.state-background-" + colorId + " > .underlay > svg { fill: " + colorValue + " !important; }");
						cssComposition.css(".pool > div > .spot.state-border-" + colorId + " { border-color: " + colorValue + " !important; }");
						cssComposition.css(".pool > div > .spot.state-background-" + colorId + " { background: " + colorValue + " !important; }");
					});
				} else if (tag === "item") {
					...
					item.children().each(function(_, i) {
						...
						if (t === "content") {
							...
							*/
							i.children("content").children().each(function(_, j) {
								j = $(j);
								/*%%
					...
					game.generalReference.setContent(kind, content, properties, modifiers);
					if (script !== null) {
						var scriptToRun = buildScript(script);
						game.triggerManager.link(triggers, function(from, itemInstance, spot, extraContext) {
							return runScript(scriptToRun)(from, itemInstance, spot, extraContext);
						});
					}
				}
			});
		...
		runScript = function(script) {
			return function(from, itemInstance, spot, extraContext, extraDrops) {
				return function(stack, buildPool, buildTrack, buildGrid, buildSpot, buildItem) {
					var context;
					...
					context = {
						...
						spot: function(location) {
							return smartBuildSpot(buildSpot(nopping, game.spotManager.getSpot(location)));
						},

						from: smartBuildSpot(buildSpot(nopping, from)),
						here: smartBuildSpot(buildSpot(nopping, spot)),
						hover: buildItem(nopping, itemInstance),
		...
		var runPrepareScript = function(end) {
			...
			var initialContext = {
				...
				item: function(kind, content, properties, modifiers, states) {
					if (states === undefined) {
						...
						game.generalReference.setContent(kind, rebuiltContent, properties, modifiers);
					}
				},

				text: function(kind, text, properties, modifiers) {
					game.generalReference.setContent(kind, $("<span>").addClass("generatedText").text(text), properties, modifiers);
				},
				color: function(kind, color, properties, modifiers) {
					game.generalReference.setContent(kind, $("<div>").addClass("generatedColor").css({ background: color }), properties, modifiers);
				},
			...
			(function(context) {
				...
				rebuiltScript += ((contents.prepare === undefined) ? ";" : contents.prepare) + '\n';
				/*
				var inRaw = false;
