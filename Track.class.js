...
class Track {
	constructor(layout, game, name, title, size, vertical, start, step, limit, reversed) {
		this._game = game;
		this.name = name;
		this.title = title;
		this.size = size;
		...
		layout.$.addClass("track").addClass("track-" + (vertical ? "vertical" : "horizontal")).addClass("track-" + name);
		...
		if (vertical) {
			...
			if (title !== null) {
				...
				levelLayout.layout().inside().set($("<div>").addClass("title").addClass("mainTitle").text(title));
			...
			Utils.loop(start, start + (size * step), step, function(i) {
				...
				var levelLayout = reversed ? currentContainedLayout.predd() : currentContainedLayout.add();
				var spot = new Spot(levelLayout.layout().inside(), game, locationPrefix + level, { overlayable: true });
				game.spotManager.registerSpot(spot);
				levelLayout.layout().west().set($("<div>").addClass("title").text("" + i));
				that.spots.push({
					layout: levelLayout,
					spot: spot
				});
			...
			Utils.loop(start, start + (size * step), step, function(i) {
				...
				var levelLayout = currentContainedLayout.add();
				var spot = new Spot(levelLayout.layout().inside(), game, locationPrefix + level, { overlayable: true, layout: "vertical" });
				game.spotManager.registerSpot(spot);
