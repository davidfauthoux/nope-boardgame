	...
	constructor(game) {
		...
		var _runScripts = function(overflowCount, scripts, liveId) {
			...
			Utils.each(scripts, function(script) {
				script(stack(false, liveId), scriptContext.buildPool, scriptContext.buildTrack, scriptContext.buildGrid, scriptContext.buildSpot, scriptContext.buildItem);
				console.log("Finishing script (" + scriptContext.caughtSpots.length + " spots modified)");

				Utils.each(scriptContext.caughtSpots, function(spot) {
					game.triggerManager.scriptRun(spot, function(scripts) { _runScripts(overflowCount + 1, scripts, liveId); });
				});
			});
		};
		...
		this._stackAndTrigger = function(event, from, spot, item, liveId) {
			...
			game.triggerManager.spotModified(from, spot, item, function(scripts) { _runScripts(0, scripts); });
			_finish(liveId);
		};
...
ExecutionContext.stackAndTrigger = function(game, event, from, spot, item, liveId) {
